...
export async function downloadAndInstall(release: KitRelease): Promise<string> {
    ...
    const files = release.assets.filter(filterAssetsByOS).filter(filterAssetsByArch);
    if (files.length === 0) {
        throw new Error("No matching release found for this OS and architecture");
    }
    const file = files[0];
    const downloadPath = await downloadFile(file);
    const checksum  = release.assets.find(asset => asset.archiveFilename.includes('checksums')) as KitArchiveFile
