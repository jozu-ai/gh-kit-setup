 ...
 */
export async function verifyHash(downloadedArchivePath: string, file: KitArchiveFile): Promise<void> {
    const hashes = await getPublishedHash(file)
    if (!hashes) {
        return
    }
   const correctHash = hashes.find( (hash: { hashFileName: string; }) => downloadedArchivePath.endsWith(hash.hashFileName))

    const actualHash = await hashFile(downloadedArchivePath, "sha256");
    ghCore.debug(`Correct hash for ${file.archiveFilename} is ${correctHash?.hash}`);
 ...
 */
async function getPublishedHash(file: KitArchiveFile): Promise<HashFileContents[] | undefined>{
    const httpc = new http.HttpClient();
    const resp = await httpc.get(file.archiveFileUrl, { "Content-Type": "text/plain; charset=utf-8" });
